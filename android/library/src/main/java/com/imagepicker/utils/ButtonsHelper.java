...
public class ButtonsHelper
{
    ...
    public static ButtonsHelper newInstance(@NonNull final JSONObject options)
    {
    ...
    private static @Nullable Item getItemFromOption(@NonNull final JSONObject options,
    ...
    private static @NonNull LinkedList<Item> getCustomButtons(@NonNull final JSONObject options)
    {
        LinkedList<Item> result = new LinkedList<>();
        if (!options.containsKey("customButtons"))
        {
            return result;
        }

        final JSONArray customButtons = options.getJSONArray("customButtons");
        for (int i = 0; i < customButtons.size(); i++)
        {
            final JSONObject button = customButtons.getJSONObject(i);
